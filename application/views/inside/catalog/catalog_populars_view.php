          ...
          <div class="product_add_content clearfix">
            <p>
              <a class="product--detail_like operation-link on <?php
                   $cookie = (isset($_COOKIE[$this->config->item('cookie_favorite')])) ? explode(',', $_COOKIE[$this->config->item('cookie_favorite')]) : array();
                   if (in_array($value['id'], $cookie)) echo 'active'; ?>" 
              href="<?php echo anchor_wta(site_url('ajax/operation/favorite')); ?>"
               data-post="product=<?php echo $value['id']; ?>" title="<?php echo $value['name']; ?>">
                <span class="product--detail_like__quant"><?php echo $value['favorite_count']; ?></span>
              </a>
            </p>
            <input type="hidden" id="count_like_for_object" value="<?php echo $value['favorite_count']; ?>">
            <input type="hidden" id="id_like_object" value="<?php echo $value['id']; ?>">
            <!-- лайки -->
