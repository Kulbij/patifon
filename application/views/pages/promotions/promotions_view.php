<div id="cnt">
 ...
 <?php
  if (isset($SITE_CONTENT['content']) && !empty($SITE_CONTENT['content'])) :
   ...
   $last_promotions = array();
   if ($PAGINATION['THISPAGE'] == 1) {

    $last_promotions = array_slice($SITE_CONTENT['content'], 0, 3);
 ...
 ?>
  ...
  <div class="ar">
   ...
   <?php if (isset($last_promotions) && !empty($last_promotions)) : ?>

    <?php foreach ($last_promotions as $key => $value) : ?>

     <?php if ($key == 0) : ?>

      <div class="arbgit">
        <a class="itim" href="<?php echo anchor_wta(site_url('promotions/detail/'.$value['link'])); ?>">
         <span class="imvral">
        ...
        <div class="ittt">
         <a class="ttlk" href="<?php echo anchor_wta(site_url('promotions/detail/'.$value['link'])); ?>">
          <?php echo $value['name']; ?>
         </a>
        </div>
     ...
     <?php else : ?>
      ...
      <a class="arslit" href="<?php echo anchor_wta(site_url('promotions/detail/'.$value['link'])); ?>">
        <span class="itvral">
   ...
   <div class="pg">
     ...
     <ul class="pgls">
      ...
      <?php for ($i = $start_; $i <= $finish_; ++$i) : ?>
       ...
       <?php else : ?>
        <li class="lsit"><?php echo anchor(site_url('promotions/page/'.$i), $i, array('class' => 'itlk')); ?></li>
       <?php endif; ?>
     ...
     <a
      <?php if ($PAGINATION['THISPAGE'] > 1) : ?>
       href="<?php echo anchor_wta(site_url('promotions/page/'.($PAGINATION['THISPAGE'] - 1))); ?>" class="pgpr"
      <?php else : ?>
     ...
     <a
      <?php if ($PAGINATION['THISPAGE'] < $PAGINATION['COUNTPAGE']) : ?>
       href="<?php echo anchor_wta(site_url('promotions/page/'.($PAGINATION['THISPAGE'] + 1))); ?>" class="pgnx"
      <?php else : ?>
