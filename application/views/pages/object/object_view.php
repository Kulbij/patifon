...
<div class="container">
	...
	<?php endif; ?>
			...
			<?php if(isset($SITE_CONTENT['images']) && !empty($SITE_CONTENT['images']) || 
				...
				<?php foreach($SITE_CONTENT['object']['filters'] as $value) : ?>
					<?php if(isset($value['image']) && !empty($value['image']) && $value['class'] != 'icpc') : ?>
						<li>
						<?php if($value['class'] == 'icopss') : ?>
							<img src="<?php echo baseurl($value['image']); ?>" alt="">
							<span><?php echo $value['name']; ?></span>
						<?php elseif($value['class'] == 'iccam') : ?>
							<img src="<?php echo baseurl($value['image']); ?>" alt="">
							<span><?php echo $value['camara_count']; ?></span>
						<?php else : ?>
							<img src="<?php echo baseurl($value['image']); ?>" alt="">
						<?php endif; ?>
						</li>
				...
				<?php if (isset($SITE_CONTENT['colors']) && !empty($SITE_CONTENT['colors'])) : ?>
					...
					<?php endif; ?>
						...
						<form action="<?php echo anchor_wta(site_url('ajax/form-send/buyback')); ?>" method="post" class="form_buy_one_click">
							<div>
								<input type="text" class="buy_one_click__input" name="phone" placeholder="+38 (___) ___–__–__"  required></div>
							<div>
					...
					<!-- О ДОСТАВКЕ/ГАРАНТИИ ТОВАРА -->
								...
								<?php $price_old = $SITE_CONTENT['object']['price'];
									...
									<section class="delivery">
										...
										<div class="style_info">Из магазина Ровно — бесплатно</div>

										<div class="style_info">
											<p>Регионы – &laquo;Нова пошта&raquo;</p>

											<div class="product_info_table">
												<div>Наличный платеж</div>
												<div>+<?php echo $new_price; ?> грн.</div>
											</div>
							...
							<?php if(isset($SITE_CONTENT['product_info']) && !empty($SITE_CONTENT['product_info'])) : ?>
								<?php foreach($SITE_CONTENT['product_info'] as $value) : ?>
								<!-- ГАРАНТИЯ -->
									...
									<?php if(isset($value['text']) && !empty($value['text'])) : ?>
										<div class="delivery-info">
											...
											<div class="drop">
							                  ...
							                  <div class="drop-text">
												<php <?php echo $value['text']; ?>
							                  </div>
							                </div>
										</div><!-- end .delivery-info -->
									<?php endif; ?>

										<?php if(isset($value['shorttext']) && !empty($value['shorttext'])) echo $value['shorttext']; ?>
									</section>
									<!-- END ГАРАНТИЯ -->
						...
						<?php if(isset($SITE_CONTENT['cat_option']['share']) && !empty($SITE_CONTENT['cat_option']['share'])) : ?>
									<div class="style_info">
										<div class="select_block_inner">
											<div>
												<label for="select_v_1">Прокачай гаджет</label>
						...
						<?php if(isset($SITE_CONTENT['more_option']['share']) && !empty($SITE_CONTENT['more_option']['share'])) : ?>
									<div class="style_info">
										<div class="select_block_inner">
											<div>
												<label for="select_v_2">Дополнительно</label>
		...
		<?php endif; ?>
					...
					<div id="subs-review-view" name="tab_detail_otvet">
						<div class="tabs_content">
							...
							<?php if(isset($SITE_CONTENT['comments']) && !empty($SITE_CONTENT['comments'])) : ?>
					        	...
					        	<?php if($SITE_CONTENT['product_pages']['this_page'] == $SITE_CONTENT['product_pages']['count_page']) : ?>
					        		<a href="#" class="next disabled">Следующая<i class="icon"></i></a>
					        	<?php else : ?>
