...
class Auth extends Main {
 ...
 public function action_index($link = 'info', $bug = 'page', $page = 1) {
  ...
  $this->form_validation->set_rules('people', 'people', 'required|alpha_numeric|xss_clean');
  $this->form_validation->set_rules('password', 'password', 'required|xss_clean');
  $this->form_validation->set_rules('password_conf', 'password_conf', 'required|xss_clean');
  ...
  if ($this->form_validation->run() == true) {
   ...
   $proverka = $this->form_model->Proverka($data_array);

   if($proverka == 0) {
      $this->form_model->insertUser($data_array);
      $user_id = $this->form_model->getUserFullRegistration($data_array);
      echo $user_id; die();
   ...
   if (!empty($temp)) $errors['password_conf'] = '!';
  ...
  public function good_reg() {
    ...
    $id_user = $this->form_model->getIDUser($data_array);

    echo "пользователь".$id_user." "." Вы успешер зарегмстрипрвани!";
  }
