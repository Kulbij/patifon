...
class Main extends CI_Controller {
 ...
 public function index() {
  ...
  foreach($content['banners'] as $key => $value){
    if(empty($value['object'])) unset($content['banners'][$key]);
  }
  ...
  $link = [];
  $link = 'all';
  $content['catalognew'] = $this->catalog_model->getPopular(array(), 8, $link);
  foreach($content['catalognew'] as &$one){
      $one['comm_count'] = $one['comm_count'] . ' ' . $this->pluralForm($one['comm_count'], 'отзыв', "отзыва", "отзывов");
      $one['mark'] = $this->catalog_model->getObjectAvgMark($one['id']);
      $one['links'] = $this->menu_model->getObjectCategories($one['id']);
  }
 ...
 protected function generateTop() {
  ...
  $url = explode('.', $url);
  $url = $url[0];
  $return['url'] = $this->menu_model->geturl($url);

  // main category for manu

    @$url_menu = explode('/', $_SERVER['REDIRECT_QUERY_STRING']);
    if(isset($url_menu[2]) && !empty($url_menu[2])) $url_menu = explode('.', $url_menu[2]);

    $return['url_memu'] = $url_menu[0];

  // end categry for main

  // ------ add url address for category object
  $ulr_category = explode('/', $_SERVER['REQUEST_URI']);

  if(isset($ulr_category[3]) && !empty($ulr_category[3])){
    if($ulr_category[2] == ''){
      $return['ulr_category'] = $ulr_category[3];
    } else {
      $return['ulr_category'] = $ulr_category[2];
    }
  }
  // ------------end add url ----------------------
  ...
  $more_phones = $return['phones'];
  unset($more_phones[0]);
  $return['more_phones'] = $more_phones;
...
//  die();
 ...
 public function watermark($objid = null, $t="", $image = null) {        
  if (!is_null($objid) && !is_null($image) && file_exists($_SERVER['DOCUMENT_ROOT'] . '/images/'. $objid . '/'.$t.'/' . $image)) {     
    
      $sizer = getimagesize($_SERVER['DOCUMENT_ROOT'] . '/images/'. $objid . '/'.$t.'/' . $image);
      ...
      if ($sizer[0] > 300 && $sizer[1] > 200) {
          ...
          $config = array();
          $config['source_image'] = $_SERVER['DOCUMENT_ROOT'] . '/images/'. $objid . '/'.$t.'/' . $image;
