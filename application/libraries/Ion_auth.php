...
class Ion_auth
{
   ...
   $user = $this->where($this->config->item('identity', 'ion_auth'), $identity)->where('active', 1)->users()->row();  //changed to get_user_by_identity from email
     ...
     $data['password'] = $this->input->generate_string($data['identity'], $this->config->item('min_password_length', 'ion_auth'));
     $change = $this->ion_auth->reset_password($data['identity'], $data['password']);
     ...
     if ($change) {
      ...
      $send_array = array(
       'phone' => $this->input->prepare_user_identity_phone($data['identity']),
       'desc' => $this->lang->line('ap_fp_send_desc'),
       'text' => sprintf($this->lang->line('ap_fp_send_text'), $data['password'])
       );

      $this->load->library('send_lib');
      if ($this->send_lib->send_sms($send_array)) {
       return true;
      } else {
    ...
    // $this->load->library('send_lib');
    // if ($this->send_lib->user_send_activation($data)) {
    //  return $id;
    // }
