...
class CI_Cart {
	...
	var $product_name_rules	= '\.\:\-_ a-z0-9а-ЯыЫъЪёЁэЭєЄїЇіІ'; // alpha-numeric, dashes, underscores, colons or periods
	...
	function _insert($items = array())
	{
		...
		$items['qty'] = trim(preg_replace('/([^0-9])/iu', '', $items['qty']));
		// Trim any leading zeros
		$items['qty'] = trim(preg_replace('/(^[0]+)/iu', '', $items['qty']));
		...
		$items['price'] = trim(preg_replace('/([^0-9\.])/iu', '', $items['price']));
		// Trim any leading zeros
		$items['price'] = trim(preg_replace('/(^[0]+)/iu', '', $items['price']));
	...
	function _update($items = array())
	{
		...
		// Prep the quantity
		$items['qty'] = preg_replace('/([^0-9])/iu', '', $items['qty']);
