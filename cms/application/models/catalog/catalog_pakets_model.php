...
class Catalog_pakets_model extends CI_Model
{
    ...
    public function setVisible($id) {
        ...
        $this->db->where("id = '{$id}'")->update('site_pakets');
    }
    ...
    public function selectPreviewAll($page = 1, $count = 100, $category = 0) {
        ...
        $this->db->select('site_pakets.id, site_pakets.name_ru as name, site_pakets.visible, site_pakets.position, site_pakets.image');
        
        $this->db->from('site_pakets');
        $res = $this->db->order_by('site_pakets.position', 'ASC')->get();
    ...
    public function inOption($array = array()){
        ...
        if(isset($array['image']) && !empty($array['image'])) $this->db->set('image', $array['image']);

        $this->db->insert('site_pakets');
    ...
    public function upOption($array = array()){
        ...
        if(isset($array['id']) && !empty($array['id'])){
            $this->db->where('site_pakets.id', $array['id']);

            $this->db->update('site_pakets');
            $return = $array['id'];
        }
    ...
    public function gererateIDOpttion($id = 0){
        ...
        $res = $this->db->select('site_pakets.id')->where('site_pakets.id', $id)->from('site_pakets')->get();
    ...
    public function getOne($id = 0){
        ...
        $res = $this->db->select('site_pakets.id, site_pakets.name_'.SITELANG.' as name, site_pakets.image')
        ->where('site_pakets.id', $id)->from('site_pakets')->limit(1)->get();
    ...
    public function remove($id){
        ...
        $this->db->where('site_pakets.id', $id)->delete('site_pakets');
    }
    ...
    public function sort_paket_option($array) {
        if (!empty($array)) {
            foreach ($array['sort'] as $key => $val) {
                ...
                $this->db->update('site_pakets', array('position' => $val));
            }
        }
    }
