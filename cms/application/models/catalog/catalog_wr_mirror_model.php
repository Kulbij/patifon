...
class Catalog_wr_mirror_model extends CI_Model
{
    ...
    public function setVisible($id) {
        ...
        $this->db->where("id = '{$id}'")->update('site_wardrobe_mirror');
    }
    ...
    public function setUp($id) {
     $res = $this->db->select('position')->from('site_wardrobe_mirror')->where("id = '{$id}'")->limit(1)->get();
     ...
     $res = $this->db->select('id, position')->from('site_wardrobe_mirror')->where("position < '{$pos['position']}'")->limit(1)->order_by('position', 'DESC')->get();
     $res = $res->result_array();
     
     if (count($res) <= 0) return false;
     
     $this->db->set('position', $res[0]['position'])->where("id = '{$id}'")->update('site_wardrobe_mirror');
     $this->db->set('position', $pos['position'])->where("id = '{$res[0]['id']}'")->update('site_wardrobe_mirror');
    }
    ...
    public function setDown($id) {
     ...
     $res = $this->db->select('id, position')->from('site_wardrobe_mirror')->where("position > '{$pos['position']}'")->limit(1)->order_by('position', 'ASC')->get();
     $res = $res->result_array();
    ...
    public function selectPreviewAll() {
        ...
        $this->db->from('site_wardrobe_mirror');
        ...
        */
        ...
        if (isset($array['id']) && is_numeric($array['id'])) {
            ...
            */
            ...
            $this->db->where("id = '{$array['id']}'")->update('site_wardrobe_mirror');
            $objectid = $array['id'];
            ...
            $this->db->insert('site_wardrobe_mirror');
            $objectid = $this->db->insert_id();
            
            #if (!$array['manual']) $this->db->set('link', rtrim($array['link'], '-').'-'.$objectid);
            $this->db->set('position', $objectid)->where('id = '.$this->db->escape($objectid))->update('site_wardrobe_mirror');
            
        }
        
        if (isset($array['image']['image']) && !empty($array['image']['image'])) {
         $this->load->library('image_my_lib');
         $res = $this->db->select('image')->from('site_wardrobe_mirror')->where("id = '{$objectid}'")->limit(1)->get();
         ...
         $this->db->set('image', $array['image']['image'])->where('id = '.$this->db->escape($objectid))->update('site_wardrobe_mirror');
        }
        
        if (isset($array['image']['image_big']) && !empty($array['image']['image_big'])) {
         $this->load->library('image_my_lib');
         $res = $this->db->select('image_big')->from('site_wardrobe_mirror')->where("id = '{$objectid}'")->limit(1)->get();
         ...
         $this->db->set('image_big', $array['image']['image_big'])->where('id = '.$this->db->escape($objectid))->update('site_wardrobe_mirror');
        }
    ...
    public function deleteV($id) {
     ...
     $this->load->library('image_my_lib');
     $res = $this->db->select('image, image_big')->from('site_wardrobe_mirror')->where("id = '{$id}'")->limit(1)->get();
     ...
     $this->db->where("id = '{$id}'")->limit(1)->delete('site_wardrobe_mirror');
